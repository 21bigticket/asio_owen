...
#ifdef CONFIG_ADMIN_OUT_OF_LINE
#define inline
#endif
...
inline std::optional<IssuedToken> issue_admin_token_with_pem(
    const AdminConfig& admin,
    const std::string& username,
...
inline std::optional<Principal> verify_admin_token_with_auth(
    const AdminConfig& admin,
...
inline std::optional<Principal> verify_admin_token_with_pem(
    const AdminConfig& admin,
    const std::string& auth_header,
