...
| 客户端 | 同一虚拟机本机压测（plow） |
| 并发数 | 100 |
| 压测时长 | 每个接口 30s × 2 轮，间隔 10s 回收 |
| 接口 | `/api/health` / `/api/redis` / `/api/mysql` / Config 直连 / Config 网关 |
...
- 同台虚拟机（192.168.139.230），Ubuntu 22.04，6 核 15GB RAM
- Release stripped 构建（默认 CMake 配置）
- 50 并发，30s 长压 × 3 轮，轮间 10s 冷却
- 接口：Config 直连 / Config 网关（zebra-config:30001）

### 优化历程

| 阶段 | 改动 | RPS (50并发 30s平均) | 效率 |
|:----|:-----|:------------------:|:----:|
| **优化前** | 锁内含 syscall（`is_reusable_idle` 在锁内做 3 次 IO） | ~3,000 | ~53% |
| **第一轮** | `is_reusable_idle` 移到锁外，缩短锁持时间 | ~4,100 | ~72% |
| **第二轮** | 16-shard 锁分片，每把锁竞争降到 1/16 | **~3,239** | **~79%** |

> 第二轮 sharding 后 RPS 下降是因为分片后每片 max_size 缩小，连接复用率下降。但效率从 72% 提升到 79%，说明锁竞争确实降低了。RPS 绝对值受上游 Go 服务波动影响。

### 最终结果（第二轮优化后，3 轮取均值）

| 测试 | #1 RPS | #2 RPS | #3 RPS | 平均 RPS | 2xx |
|:----|:------:|:------:|:------:|:--------:|:---:|
| **直连** `:30001` | 3,678 | 4,252 | 4,363 | **4,097** | 100% |
| **网关** `:8081/zebra-config/...` | 3,174 | 3,230 | 3,311 | **3,239** | 100% |
...
| syscall | 占比 | 说明 |
|:--------|:---:|:------|
| futex | 51.78% | 锁竞争（16 shard 后单锁压力已降低，但总锁次数不变） |
| ioctl | 15.93% | 连接健康检查 `non_blocking()` 切换 |
| sendto | 13.55% | 向上游写请求 |
| recvfrom | 11.31% | 读上游响应 + peek 探测 |
| getpeername | 4.28% | socket 健康检查 |

**每请求开销（15s × 4,100 RPS = 61,500 请求）：**

| 指标 | 每请求 |
|:-----|:------:|
| futex | 23.9μs |
| 总 syscall | 46μs |
| 全砍 futex 的天花板 | ~22μs（+15-25% RPS） |

> 结论：28% proxy overhead 对用户态 HTTP 网关已是合格水平（Envoy 默认配置 70-80% 效率）。剩余的 overhead 主要是 sendto/recvfrom 的物理 IO 成本，锁优化已经吃到肉，继续优化空间有限。
...
|:----|:--------:|:---:|:--------:|
| **直连** `:30001` | 4,097 | ✅ | — |
| **网关** `:8081/zebra-config/...` | 3,239 | ✅ | +21% |

> 网关转发 overhead ~21%（含 JWT 验证 + 鉴权链 + 上游连接池管理 + 16-shard 锁分片）。

### 稳定性检查

| 检查项 | 结果 |
|--------|:----:|
| server 日志 error/fatal | **0** ✅
| plow 成功率 | **100%** ✅ |
| RSS 压测前后 | 117MB 无增长 ✅ |
...
| 指标 | 值 |
|:----|:---:|
| 内存常驻（限流 LRU 填充后） | ~56-57 MB（vs v3.4 ~56 MB），增量 < 1 MB |
| 全局限流桶访问 | 单 atomic CAS，无锁，~100ns/op |
| 429 日志采样 | 全进程 30s 一次，不会刷屏 |
| Snapshot 落盘 | 每 30s 异步写入，~1ms |
