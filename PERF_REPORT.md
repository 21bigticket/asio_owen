...
- 100 并发，30s 长压 × 2 轮，轮间 10s 冷却
- 全部 5 接口：Health / Redis / MySQL / Config 直连 / Config 网关

### 最终结果（稳态）

| 接口 | #1 RPS | #2 RPS | 平均 RPS | 成功率 | 说明 |
|:----|:------:|:------:|:--------:|:------:|:-----|
| **Health** | 75,131 | 74,829 | **74,980** | 100% | 纯网关热路径，经鉴权链（JWT 白名单跳过）
| **Redis** | 22,685 | 22,511 | **22,598** | 100% | thread_local 连接，鉴权链影响可忽略
| **MySQL** | 7,708 | 7,688 | **7,698** | 100% | thread_pool worker，鉴权链影响可忽略
| **Config 直连** | 6,199 | 6,167 | **6,183** | 100% | 上游 Go 服务 baseline（zebra-config:30001）
| **Config 网关** | 5,492 | 5,776 | **5,634** | 100% | 经网关转发 + JWT 验证 + 全链路鉴权

### 与 v3.4 对比分析

| 接口 | v3.4 (无鉴权) | v3.5 (有鉴权) | 变化 |
|:----|:-----------:|:-----------:|:----:|
| **Health** | ~91,289 | ~74,980 | **-18%** ℹ️ |
| **Redis** | ~23,838 | ~22,598 | **-5%** |
| **MySQL** | ~7,973 | ~7,698 | **-3%** |

> Health 降幅 ~18% 来自新增的鉴权 check 链（路径规范化 + IP 黑名单 + 白名单 + 限流），属于预期开销。IO 密集型接口（Redis/MySQL）的鉴权链开销被 IO 等待时间掩盖。
...
| plow 成功率 | **100%** ✅（全部 5 接口） |
| 系统负载 | ~12-18（6 核 CPU 过载） |
| 可用内存 | ~7.3G / 15G ✅
...
|:----|:--------:|:---:|:--------:|
| **直连** `:30001` | 6,183 | ✅ | — |
| **网关** `:8081/zebra-config/...` | 5,634 | ✅ | +9% |

> 网关转发 overhead ~9%（含路由解析 + JWT 验证 + 限流 + 上游连接池管理），相比 v3.4 的 ~6% overhead 增加了约 3 个百分点，来自鉴权链的额外处理。
