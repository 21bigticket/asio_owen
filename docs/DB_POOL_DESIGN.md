...
1. 优先取 idle 连接，取出后**直接返回，不再执行 per-acquire `SELECT db` 探活**。
   db 在建连时（`create_redis_connection`，`db != 0` 时）已 SELECT 一次，且 db
   固定、切换靠重启、业务不发运行时 `SELECT`，连接的 db 状态不会漂移，因此复用时
   重复 SELECT 是多余往返。去掉它可省下 worker 模式每命令一次的 Redis 往返
   （direct→worker 吞吐回退的主因）。
...
| Redis worker 幂等重试 | 连接层失败（`reply==nullptr`）时仅对只读幂等命令白名单自动重试一次；非幂等命令失败直接返回，由调用方处理。 |
...
- **2026-07-17 定位与修复**：worker 模式回退主因是每次 acquire 复用 idle 连接前
  多发一次 `SELECT db`，对 db 固定的业务是纯多余往返，使每条 Redis 命令变成两趟
  往返。已去掉复用 SELECT，改用「建连时 SELECT + 命令失败幂等重试」兜底
  （见上文 acquire 逻辑与已知限制）。
- **2026-07-17 改后压测**（`127.0.0.1:8081`，30t/100c，30s×2）：`/api/redis`
  RPS 25632 / 24358（均值约 25.0k），avg 延迟 3.58 / 3.89ms，errors=0。较改前
  17.7k 约 +41%。⚠️ 本轮为 loopback 压测，与 17.7k/42.6k 的 VM
  `192.168.139.230`（6 核）基线是否同环境待核对，跨环境数字不可直接并列。
- 注：42.6k 是 direct 模式历史**最好单次**表现，且是在**机器冷启动、空载无其他压力**
  下测得的理想化数字，既非平均值也非稳态值，direct 稳态平均达不到该数；
  因此**不能**把它当基线，也不宜拿它与本次 worker 均值并列作差距对比。worker 相对 direct 仍有固有开销：
  每命令一次 `asio::post` 线程跳转（两次上下文切换）+ `acquire/release` 的
  `mtx_`/`cv_` 锁竞争。这两项是「同步 hiredis 隔离到独立线程池」的设计代价，
  非复用 SELECT 所致，去掉 SELECT 无法进一步消除。追求更高吞吐需改线程/锁模型或
  改用 direct 模式（放弃故障隔离）。若要量化 worker vs direct 的真实差距，应在
  同环境各压多轮取均值对比，而非对比历史峰值。
