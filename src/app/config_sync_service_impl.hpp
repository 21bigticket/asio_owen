...
public:
    ...
    ConfigSyncServiceImpl(asio::io_context& ioc, RedisPool& redis,
    ...
    ConfigSyncServiceImpl(asio::io_context& ioc, Command command,
                      std::filesystem::path config_base,
                      ConfigSyncConfig cfg, AppConfig running_app_cfg,
                      asio::thread_pool* file_workers = nullptr,
                      std::shared_ptr<ConfigSyncRuntimeMetrics> metrics = nullptr)
        ...
        run_command({"GET", std::string(kVersionKey)},
            [self, completion = std::move(completion)](Reply reply) mutable {
                ...
                if (remote_version == local_version && !history_recovery_pending) {
                    ...
                    const bool collected = self->collect_local_managed_files(
